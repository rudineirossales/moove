...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $cis = $_POST['cis'];
    $obs = $_POST['obs'];
    $capacidade_cabo = $_POST['capacidade_cabo']; // New field
    ...
    if ($stmt = $conn->prepare("INSERT INTO atividade (ba, causa, sub, ro, cis, obs, capacidade_cabo, foto_antes, foto_depois, data_encerramento) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE causa = VALUES(causa), sub = VALUES(sub), ro = VALUES(ro), cis = VALUES(cis), obs = VALUES(obs), capacidade_cabo = VALUES(capacidade_cabo), foto_antes = VALUES(foto_antes), foto_depois = VALUES(foto_depois), data_encerramento = VALUES(data_encerramento)")) {
        ...
        // Bind the parameters
        $stmt->bind_param("isssssssss", $ba, $causa, $sub, $ro, $cis, $obs, $capacidade_cabo, $_POST['foto_antes'], $_POST['foto_depois'], $hoje);
