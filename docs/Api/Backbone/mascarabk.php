...
date_default_timezone_set('America/Sao_Paulo');
    ...
    if ($stmt = $conn->prepare("INSERT INTO atividade_bbk (ba, causa, sub, ro, cis, obs, foto_antes, foto_depois, data_encerramento) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE causa = VALUES(causa), sub = VALUES(sub), ro = VALUES(ro), cis = VALUES(cis), obs = VALUES(obs), foto_antes = VALUES(foto_antes), foto_depois = VALUES(foto_depois), data_encerramento = VALUES(data_encerramento)")) {
        ...
        $sql_update_status = "UPDATE atividade_bbk SET status = 'EM VALIDACAO' WHERE ba = '$ba'";
    if ($conn->query($sql_update_status) === FALSE) {
        $response['success'] = false;
        $response['message'] = 'Error updating status in atividade_bbk table: ' . $conn->error;
        echo json_encode($response);
        $conn->close();
        exit;
    }
