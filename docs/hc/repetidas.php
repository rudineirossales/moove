<?php 
         ...
         if(!isset($_SESSION["login"]) &&  !isset($_SESSION["senha"]) )
            ...
            {
              $query ="SELECT * FROM repetida where  status is null or  status = '' "; 
            }
            else
            {
              $query ="SELECT * FROM repetida where supervisor = '".$_SESSION['nome']."' and status is null or supervisor = '".$_SESSION['nome']."' and status = '' "; 
            }
...
function fnExcelReport() {
  ...
  <body class="app sidebar-mini rtl">
    ...
    <main class="app-content">
      <div class="app-title">
        <div>
          <h1><i class="fa fa-th-list"></i> Justifica repetidas</h1>
          ...
          <div class="table-responsive">  
                     <table id="myTable" class="table table-striped table-bordered">  
                          ...
                          {  
                               echo '  
                               <tr>  
                                    <td><a href="envia_repetidas.php?sa='.$row["sa"].'"> '.$row["sa"].' </span></a></td>   
                                    <td>'.$row["uf"].'</td>
                                    <td>'.$row["cp"].'</td>
                                    <td>'.$row["mes"].'</td>  
